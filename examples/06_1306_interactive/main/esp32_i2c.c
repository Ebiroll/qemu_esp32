 ...
 * Data start with 0x40 then the strem of data
 * ______________________________________________________________________________________
 * | start | slave_addr + wr_bit + ack | write 1 byte + ack  ... write 1 byte + nack | stop |
 * --------|---------------------------|---------------------|--------------------|------|
 */
esp_err_t i2c_1306_write_data(i2c_port_t i2c_num, uint8_t* data,int len)
{
    uint8_t checksum;
    int times_tried=0;
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    ...
    // Not sure if cheap chinese displays will ack 
