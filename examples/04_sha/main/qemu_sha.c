...
bootloader_sha256_handle_t qemu_sha256_start()
{
...
void qemu_sha256_data(qemu_sha256_handle_t handle, const void *data, size_t data_len)
{
    assert(handle != NULL);
...
void qemu_sha256_finish(qemu_sha256_handle_t handle, uint8_t *digest)
{
    assert(handle != NULL);
    mbedtls_sha256_context *ctx = (mbedtls_sha256_context *)handle;
    if (digest != NULL) {
...
qemu_sha256_handle_t qemu_sha256_start()
{
    // Enable SHA hardware
    ets_sha_enable();
    words_hashed = 0;
    return (qemu_sha256_handle_t)&words_hashed; // Meaningless non-NULL value
}
...
void qemu_sha256_finish(qemu_sha256_handle_t handle, uint8_t *digest)
{
    ...
    qemu_sha256_data(handle, padding, pad_bytes);
    ...
    qemu_sha256_data(handle, &bit_count, sizeof(bit_count));
