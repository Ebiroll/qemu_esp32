...
static esp_err_t spi_lobo_bus_initialize(spi_lobo_host_device_t host, spi_lobo_bus_config_t *bus_config, int init)
{
    ...
    if (init > 0) {
        ...
        SPI_CHECK(host!=L_SPI_HOST, "SPI1 is not supported", ESP_ERR_NOT_SUPPORTED);

        SPI_CHECK(host>=L_SPI_HOST && host<=L_VSPI_HOST, "invalid host", ESP_ERR_INVALID_ARG);
...
{
	if ((host == L_SPI_HOST) || (host >L_VSPI_HOST)) return ESP_ERR_NOT_SUPPORTED;  // invalid host
