...
// 0x1000  .pioenvs/esp-wrover-kit/bootloader.bin  
// 0x10000 .pioenvs/esp-wrover-kit/firmware.bin
// 0xe000  .pioenvs/esp-wrover-kit/partitions.bin 
//
...
int main(int argc,char *argv[])
{
    ...
    // Add bootloader
    //merge_flash(".pioenvs/esp-wrover-kit/bootloader.bin","esp32flash.bin",0x1000,0);
    // Add partitions, 
    merge_flash(".pioenvs/esp-wrover-kit/partitions.bin","esp32flash.bin",0x8000,0);
    // Add application
    if (argc>1) {
      merge_flash(argv[1],"esp32flash.bin",0x10000,0);
    } else {
      merge_flash(".pioenvs/esp-wrover-kit/firmware.bin","esp32flash.bin",0x10000,0);
    }
    system("cp esp32flash.bin ~/qemu_esp32");
}
