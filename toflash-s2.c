...
int main(int argc,char *argv[])
{
    ...
    // Add bootloader
    merge_flash("build/bootloader/bootloader.bin","esp32flash.bin",0x1000,1);
    // Add partitions, 
    ...
    if (argc>1) {
      ...
      merge_flash("build/app-template.bin","esp32flash.bin",0x10000,1);
    }
    system("cp esp32flash.bin ~/qemu_esp32s2");
}
