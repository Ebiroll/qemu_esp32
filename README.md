...
The goal is to run the app-template or bootloader and connect the debugger. Connecting the debugger works quite well and as the rom is patched slightly it its possible to run the elf file that is generated by the esp-idf. However you must use the configutation 
```
...
However currently if you enter a long string with nc.
